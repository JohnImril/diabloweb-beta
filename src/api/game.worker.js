...
class RemoteFile {
	constructor(url) {
		...
		this.chunks = new Uint8Array(
			((this.byteLength + ChunkSize - 1) >> 20) | 0
		);
	}
	...
	subarray(start, end) {
		...
		if (missing0 <= missing1) {
			...
			request.setRequestHeader(
				"Range",
				`bytes=${missing0 * ChunkSize}-${Math.min(
					missing1 * ChunkSize + ChunkSize - 1,
					this.byteLength - 1
				)}`
			);
			...
			if (request.status < 200 || request.status >= 300) {
				...
				chunk1 =
					((start + request.response.byteLength + ChunkSize - 1) /
						ChunkSize) |
					0;
				for (let i = chunk0; i < chunk1; ++i) {
					this.chunks[i] = 1;
			...
			if (!websocket || websocket.readyState !== 1) {
				const sock = (websocket = websocket_open(
					...
					(data) => {
						if (websocket === sock) {
							try_api(() => {
								const ptr = wasm._DApi_AllocPacket(
									data.byteLength
								);
...
const DApi_renderOffscreen = {
	...
	draw_end() {
		...
		worker.postMessage(
			{ action: "render", batch: { bitmap, belt: drawBelt } },
			transfer
		);
		drawBelt = null;
	},
	...
	try_api(() => {
		...
		if (!nested) {
			if (audioBatch.length) {
				...
				worker.postMessage(
					{ action: "audioBatch", batch: audioBatch },
					audioTransfer
				);
			}
			if (packetBatch.length) {
				worker.postMessage(
					{ action: "packetBatch", batch: packetBatch },
					packetBatch
				);
			}
			audioBatch = null;
...
async function init_game(mpq, spawn, offscreen) {
	...
	if (!mpq) {
		...
		if (!files.has(name)) {
			...
			files.set(
				name,
				new RemoteFile(`${process.env.PUBLIC_URL}/${name}`)
			);
		}
	}
	...
	function updateProgress() {
		progress(
			"Loading...",
			mpqLoaded + wasmLoaded * wasmWeight,
			mpqTotal + wasmTotal * wasmWeight
		);
	}
