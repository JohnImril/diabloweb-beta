...
export default async function compress(mpq, progress) {
	...
	if (
		hashTablePos + hashTableSize * 16 > mpqsize ||
		blockTablePos + blockTableSize * 16 > mpqsize
	) {
		throw Error("invalid MPQ file");
	}
	...
	fHashTable.ready = readFile(
		mpq.slice(hashTablePos, hashTablePos + fHashTable.total),
		loader(fHashTable)
	);
	fBlockTable.ready = readFile(
		mpq.slice(blockTablePos, blockTablePos + fBlockTable.total),
		loader(fBlockTable)
	);
	...
	for (let task of tasks) {
		if (task.data) {
			...
			task.entries.forEach((i, pos) => {
				const index = hashTable[i * 4 + 3];
				const name =
					listMap[hashStr(hashTable[i * 4], hashTable[i * 4 + 1])];
	...
	const outputSize = tasks.reduce(
		(sum, { result }) => sum + (result ? result.buffer.byteLength : 0),
		outputPos
	);
