	...
	for (let sheet of document.styleSheets) {
		for (let rule of sheet.cssRules) {
			if (
				...
				rule.conditionText === "(min-aspect-ratio: 3/1)"
			) {
...
class App extends React.Component {
	...
	componentDidMount() {
		...
		this.fs.then((fs) => {
			...
			if (
				[...fs.files.keys()].filter((name) => name.match(/\.sv$/i))
					.length
			) {
	...
	openKeyboard(rect) {
		if (rect) {
			this.showKeyboard = {
				...
				width: `${((100 * (rect[2] - rect[0] + 20)) / 640).toFixed(
					2
				)}%`,
				height: `${((100 * (rect[3] - rect[1] + 20)) / 640).toFixed(
					2
				)}%`,
			};
	...
	updateSaves() {
		return this.fs.then((fs) => {
			...
			[...fs.files.keys()]
				...
				.forEach((name) => {
					saves[name] = getPlayerName(
						fs.files.get(name).buffer,
						name
					);
				});
	...
	mousePos(e) {
		...
		if (this.pointerLocked()) {
			this.cursorPos.x = Math.max(
				rect.left,
				Math.min(rect.right, this.cursorPos.x + e.movementX)
			);
			this.cursorPos.y = Math.max(
				rect.top,
				Math.min(rect.bottom, this.cursorPos.y + e.movementY)
			);
		} else {
	...
	onKeyboardInner(flags) {
		if (this.showKeyboard) {
			...
			if (this.maxKeyboard > 0) {
				valid = (text.match(/[\x20-\x7E]/g) || [])
					.join("")
					.substring(0, this.maxKeyboard);
			} else {
				const maxValue = -this.maxKeyboard;
				if (text.match(/^\d*$/)) {
					this.keyboardNum = Math.min(
						text.length ? parseInt(text) : 0,
						maxValue
					);
				}
	...
	updateTouchButton(touches, release) {
		...
		for (let { target, identifier, clientX, clientY } of touches) {
			if (
				btn &&
				btn.id === identifier &&
				this.touchButtons[btn.index] === target
			) {
				if (touches.length > 1) {
					btn.stick = false;
				}
		...
		this.touchCanvas = [...touches].find(
			(t) => !touchOther || t.identifier !== touchOther.id
		);
		if (this.touchCanvas) {
			this.touchCanvas = {
				clientX: this.touchCanvas.clientX,
	...
	renderUi() {
		...
		if (show_saves && typeof save_names === "object") {
			...
			return (
				<div className="start">
					...
					<div
						...
						onClick={() => this.setState({ show_saves: false })}
					>
						Back
					</div>
				</div>
			...
			return (
				<Link
					className="error"
					href={reportLink(error, this.state.retail)}
				>
			...
			return (
				<div className="start">
					<p>
						This is a web port of the original Diablo game, based on
					...
					<p>
						If you own the original game, you can drop the original
						DIABDAT.MPQ onto this page or click the button below to
						start playing. The game can be purchased from{" "}
						<Link href="https://www.gog.com/game/diablo">GoG</Link>.{" "}
						<span
							className="link"
							onClick={() => this.setState({ compress: true })}
						>
							Click here to compress the MPQ, greatly reducing its
							size.
						</span>
					</p>
					{!has_spawn && (
						<p>
							Or you can play the shareware version for free (50MB
							download).
						</p>
					)}
					<form>
